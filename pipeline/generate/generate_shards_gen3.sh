...
if [ "$MAX_SHARDS" -gt 0 ]; then
  ...
  echo "=== assert pool sizing: target $MAX_SHARDS shards x $GAMES_PER_SHARD games, measured consumption factor=$CONSUMPTION_FACTOR => need normal=$NEED_NORMAL (has $HAVE_NORMAL), endgame=$NEED_ENDGAME (has $HAVE_ENDGAME) ==="
...
while true; do
  ...
  echo "=== $SID: self-play ($GAMES_PER_SHARD games, $NODES_SELFPLAY nodes, gen2 network) ==="
